...
Researchers and policymakers would like to know the degree to which gaps across social categories (e.g. race, class, gender) would close if we intervened to modify a treatment variable (e.g. incarceration, education, occupation). This package provides software to support these gap-closing estimands. The user specifies the category that defines a disparity, the intervention to a binary treatment variable that could help to close that disparity, and models for treatment assignment and or expected outcomes that include covariates to identify the effect of the treatment. The package supports estimation by generalized linear models, ridge regression, generalized additive models, and random forests with and without cross-fitting. This vignette focuses on the software; for additional conceptual details, see the accompanying paper.
...
In a data frame `data`, we have a gap-defining variable (category) such as race, gender, or class; we will never posit a world in which the gap-defining variable were different for any individual. Instead, gap-closing estimands view the category as simply a label on individuals. We care about the disparity in an outcome across those with different category labels.
...
To what degree would the outcome gaps between categories A, B, and C close if we intervened on the treatment `treatment`? A call to `gapclosing()` answers this question.
