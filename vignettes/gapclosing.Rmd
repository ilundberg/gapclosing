...
```{r, fig.width = 10, fig.height = 7, out.width = "650px", fig.cap = "Figure 1 produced by plot() function"}
plots <- plot(estimate, return_plots = TRUE)
print(plots[[1]])
```

The remaining plots directly examine the factual and counterfactual disparity across two categories, such as categories A and B. In this case, the intervention closes the gap between categories A and B because it has a large positive effect in category A and close to zero effect in category B, thus approximately equalizing the outcomes across these categories.
```{r, fig.width = 10, fig.height = 7, out.width = "650px", fig.cap = "Figure 1 produced by plot() function"}
print(plots[[2]])
```
The above can also be generated by the `disparityplot()` function, which allows the user to specify any specific pair of categories of interest: `disparityplot(estimate, category_A = "A", category_B = "B")`.
...
gridExtra::grid.arrange(
  plot(estimate, return_plots = T)[[1]] +
    ggtitle("GLM estimate") +
    ylim(c(-1.5,1.5)),
  plot(estimate_gam, return_plots = T)[[1]] +
    ggtitle("GAM estimate") +
    ylim(c(-1.5,1.5)),
  plot(estimate_ranger, return_plots = T)[[1]] +
